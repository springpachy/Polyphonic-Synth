...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE transmitbuffer[2] = {reg +2, val};
	int status;
	status = XSpi_Transfer(&SpiInstance, transmitbuffer, NULL, 2);
    if (status != XST_SUCCESS) {
        xil_printf("Error: SPI single write failed for register 0x%X\n", reg);
    }
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int size=1+nbytes;
	BYTE transmitbuffer[size];
	transmitbuffer[0] = reg+2;
	for (int i =0; i< nbytes; i++){
		transmitbuffer[i+1] = data[i];
	}
	int status;
	status = XSpi_Transfer(&SpiInstance, transmitbuffer, NULL, size);
	if (status != XST_SUCCESS) {
	    xil_printf("Error: SPI multibyte write failed for register 0x%X\n", reg);
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0x0);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE transmitbuffer[2] = {reg, 0};
	BYTE receivebuffer[2]= {0};
	int status;
	status = XSpi_Transfer(&SpiInstance, transmitbuffer, receivebuffer, 2);
	if (status != XST_SUCCESS) {
		xil_printf("Error: SPI single read failed for register 0x%X\n", reg);
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0x0);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
	XSpi_SetSlaveSelect(&SpiInstance, 0x1);
	BYTE transmitbuffer[1 + nbytes];
	BYTE receivebuffer[1 + nbytes];
	transmitbuffer[0] = reg;
	for (int i = 1; i < 1 + nbytes; i++) {
		transmitbuffer[i] = 0;
	}
	int status;
	status = XSpi_Transfer(&SpiInstance, transmitbuffer, receivebuffer, 1 + nbytes);

	if (status != XST_SUCCESS) {
		xil_printf("Error: SPI multi byte read failed for register 0x%X\n", reg);
	}
	for (int i = 0; i < nbytes; i++) {
...
/* probe bus to determine device presence and speeddjhhhhh */
